...
class regex:
  ...
  def __call__(self, *args):
    def regex_method(text=None):
      return [x for x in self.regex.findall(text)]
    return regex_method

class RegexAnalyzer(object):

    def __init__(self, regexfile):
        
        for k, v in list(regexes.items()):
            setattr(self, k, regex(v)(self))

        if regexfile and regexfile.endswith('.json'):
          with open(regexfile, 'r') as otherRegrexFile:
             expressionList = json.load(otherRegrexFile)
             for exp in expressionList:
               if (exp['name'] and exp['expression']):
